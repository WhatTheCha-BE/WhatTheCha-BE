...
router.post('/content/want', async (req, res) => {
  try {
    ...
    const existprofileName = await Profile.findOne(
      { profileName },
      { _id: false, want: true }
    );
    ...
    const want = await Content.find(
      { movieId: wantMovieId },
      { _id: false, movieId: true, card_image: true }
    );
    res.status(200).json({
      want,
      ok: true,
...
router.post('/content/continue', async (req, res) => {
  try {
    const { profileName } = req.body;
    const existlistRelay = await Profile.findOne(
      { profileName },
      { _id: false, listRelay: true }
    );
    const listRelayMovieId = existlistRelay.listRelay;
    const listRelay = await Content.find(
      { movieId: listRelayMovieId },
      { _id: false, movieId: true, card_image: true }
    );
...
router.post('/content/complete', async (req, res) => {
  try {
    const { profileName } = req.body;
    const existcomplete = await Profile.findOne(
      { profileName },
      { _id: false, complete: true }
    );
    const completeMovieId = existcomplete.complete;
    const complete = await Content.find(
      { movieId: completeMovieId },
      { _id: false, movieId: true, card_image: true }
    );
...
router.post('/content/doneEvaluation', async (req, res) => {
  try {
    ...
    const { profileName } = req.body;
    const existdoneEvaluation = await Profile.findOne(
      { profileName },
      { _id: false, doneEvaluation: true }
    );

    //want안에 movieId를 기준으로 card_image를 찾는다.
    const doneEvalMovieId = existdoneEvaluation.doneEvaluation;
    const doneEvaluation = await Content.find(
      { movieId: doneEvalMovieId },
      { _id: false, movieId: true, card_image: true }
    );
...
router.post('/content/detail/movieId/want', async (req, res) => {
  try {
    ...
    const myprofileName = await Profile.findOne(
